...
func TestAuthSecurityIntegration(t *testing.T) {
	t.Run("development allows plaintext login for local web console", func(t *testing.T) {
		...
		rec := loginWithStatus(t, router, "/api/client/v1/auth/login", "admin", "admin123456")
		if rec.Code != http.StatusOK {
	...
	t.Run("expired access token is rejected and refresh token can renew session", func(t *testing.T) {
		...
		tokens := login(t, router, "/api/client/v1/auth/login", encryptForPublicKey(t, publicKey, "admin123456"))
	...
	t.Run("login endpoint bans IP after ten failures", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			rec := loginWithStatus(t, router, "/api/client/v1/auth/login", "admin", encryptedWrongPassword)
			if rec.Code != http.StatusUnauthorized {
	...
	t.Run("login requires valid captcha", func(t *testing.T) {
		...
		rec := loginWithStatusWithCaptcha(t, router, "/api/client/v1/auth/login", "admin", "admin123456", "missing", "0000")
		if rec.Code != http.StatusBadRequest {
