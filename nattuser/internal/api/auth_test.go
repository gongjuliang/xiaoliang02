...
func TestAuthFlow(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/client/v1/auth/me", nil)
...
func TestAuthSecurityIntegration(t *testing.T) {
	t.Run("development allows plaintext login for local web console", func(t *testing.T) {
		...
		rec := loginWithStatus(t, router, "/api/client/v1/auth/login", testAdminUsername, testAdminPassword)
		if rec.Code != http.StatusOK {
	...
	t.Run("expired access token is rejected and refresh token can renew session", func(t *testing.T) {
		...
		tokens := login(t, router, "/api/client/v1/auth/login", encryptForPublicKey(t, publicKey, testAdminPassword))

		rec := authMe(t, router, "/api/client/v1/auth/me", "Bearer "+tokens.AccessToken)
		if rec.Code != http.StatusUnauthorized {
			t.Fatalf("expired access token status=%d body=%s", rec.Code, rec.Body.String())
		}

		renewed := refresh(t, router, "/api/client/v1/auth/refresh", tokens.RefreshToken)
	...
	t.Run("protected endpoints require bearer access token", func(t *testing.T) {
		...
		for name, header := range map[string]string{
			...
			rec := authMe(t, router, "/api/client/v1/auth/me", header)
			if rec.Code != http.StatusUnauthorized {
	...
	t.Run("login endpoint bans IP after ten failures", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			rec := loginWithStatus(t, router, "/api/client/v1/auth/login", testAdminUsername, encryptedWrongPassword)
			if rec.Code != http.StatusUnauthorized {
	...
	t.Run("login requires valid captcha", func(t *testing.T) {
		...
		rec := loginWithStatusWithCaptcha(t, router, "/api/client/v1/auth/login", testAdminUsername, testAdminPassword, "missing", "0000")
		if rec.Code != http.StatusBadRequest {
