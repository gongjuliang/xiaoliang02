...
func TestRouterServesEmbeddedFrontend(t *testing.T) {
	...
	if !strings.Contains(index.Body.String(), "sessionStorage") || !strings.Contains(index.Body.String(), "natt_client_active_view") {
	...
	if !strings.Contains(tunnels.Body.String(), "状态详情") || !strings.Contains(tunnels.Body.String(), "last_error") {
		t.Fatalf("tunnels page missing status detail column: %s", tunnels.Body.String())
	}
	for _, want := range []string{"控制端口", "数据端口", "公网监听端口", "remote_port", "服务端秘钥", "maskSecret", "show-secret", "renderDetailText", "show-detail"} {
		if !strings.Contains(tunnels.Body.String(), want) {
			t.Fatalf("tunnels page missing tunnel display marker %q", want)
		}
	}
	...
	for _, want := range []string{"如何使用 MCP", "/mcp/tools/call", "client.list_tunnel_connections", "client.connect_server", "保存成功", "check-row"} {
