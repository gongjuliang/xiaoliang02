...
</html>`, clientBrandName, clientBrandName, selectedAttr(cfg.App.Environment, "development"), selectedAttr(cfg.App.Environment, "production"), cfg.HTTP.Port, cfg.ServerDefaults.ControlPort, cfg.ServerDefaults.DataPort, html.EscapeString(cfg.ServerDefaults.ServerHost), html.EscapeString(cfg.Database.Path), checkedAttr(cfg.HTTP.HTTPSEnabled))
}
