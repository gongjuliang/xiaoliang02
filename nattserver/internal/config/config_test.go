...
func TestDefaultUsesNewStartupPortsAndTunnelRange(t *testing.T) {
	...
	if DefaultPath != filepath.Clean("xiaoliang02_server/config/config.json") {
		t.Fatalf("DefaultPath=%q want xiaoliang02_server/config/config.json", DefaultPath)
	}
	if LegacyYAMLPath != filepath.Clean("xiaoliang02_server/config/config.yaml") {
		t.Fatalf("LegacyYAMLPath=%q want xiaoliang02_server/config/config.yaml", LegacyYAMLPath)
	}
	...
	if cfg.HTTP.CertFile != filepath.Clean("xiaoliang02_server/ssl/web.crt") || cfg.HTTP.KeyFile != filepath.Clean("xiaoliang02_server/ssl/web.key") {
		t.Fatalf("default HTTPS files=%q,%q want xiaoliang02_server/ssl/web.crt,xiaoliang02_server/ssl/web.key", cfg.HTTP.CertFile, cfg.HTTP.KeyFile)
	}
	if cfg.Database.Path != filepath.Clean("xiaoliang02_server/data/nattserver.db") {
		t.Fatalf("database.path=%q want xiaoliang02_server/data/nattserver.db", cfg.Database.Path)
	}
	if cfg.Log.Dir != filepath.Clean("xiaoliang02_server/logs") {
		t.Fatalf("log.dir=%q want xiaoliang02_server/logs", cfg.Log.Dir)
	}
	if cfg.Auth.SM2PrivateKeyFile != filepath.Clean("xiaoliang02_server/data/sm2_private.pem") ||
		cfg.Auth.SM2PublicKeyFile != filepath.Clean("xiaoliang02_server/data/sm2_public.pem") {
...
func TestLoadDefaultPrefersJSONAndRequiresJSON(t *testing.T) {
	t.Run("prefers json", func(t *testing.T) {
		dir := t.TempDir()
		if err := os.MkdirAll(filepath.Join(dir, "xiaoliang02_server", "config"), 0o755); err != nil {
			t.Fatalf("create config dir: %v", err)
		}
		if err := os.WriteFile(filepath.Join(dir, "xiaoliang02_server", "config", "config.yaml"), []byte("app:\n  name: yaml-default\n"), 0o644); err != nil {
			t.Fatalf("write yaml config: %v", err)
		}
		if err := os.WriteFile(filepath.Join(dir, "xiaoliang02_server", "config", "config.json"), []byte(`{"app":{"name":"json-default"}}`), 0o644); err != nil {
